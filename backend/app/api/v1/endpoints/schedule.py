...
async def generate_schedule(
    ...
    Бизнес-логика:
    - Использует Генетический Алгоритм (GA) для поиска глобально оптимального расписания
    - Применяет систему штрафов за просрочку, раннюю готовность и технологические разрывы
    - Учитывает доступность оборудования и компетенции персонала
    - Автоматически выявляет дефицит ингредиентов на складе
    
    Пример использования:
    Инженер по планированию нажимает "Сформировать план" в разделе "Планирование".
    Система создаёт оптимальное расписание.
    """
...
async def get_schedule_calendar(
    ...
    resource_type: str = Query(default="equipment", pattern="^(equipment|employee)$", description="Тип ресурса для группировки"),
    db: AsyncSession = Depends(get_db),
) -> dict:
    """
